...
def process_value(value, data_type, custom_fn=None):
    if data_type == "string":
        return value
...
def simple_app_layout(component):
    return [
...
def simple_app_callback(
    ...
    _start_app_server(
        app, dash_duo, component_id, test_prop_name, test_prop_value, take_snapshot
    )


def nested_component_layout(component):
    return [
        dcc.Input(id="prop-name"),
        dcc.Input(id="prop-value"),
        html.Div(id="pass-fail-div"),
        html.Button("Submit", id="submit-prop-button"),
        dcc.Graph(id="test-graph", figure=component),
    ]
...
def nested_component_app_callback(
    ...
    def setup_click_callback(nclicks, prop_value):
        if nclicks is not None and nclicks > 0:
            component_props[test_prop_name] = process_value(prop_value, prop_value_type)
    ...
    def nested_callback(fig, nclicks):
        ...
        if path_to_test_prop:
            ...
            passfail = (
                PASS
                if json.dumps(eval("fig" + path_to_test_prop)) == test_prop_value
                else FAIL
            )
        else:
...
def user_interactions_layout(component):
    return [component, html.Div(id="interaction-results")]


def user_interactions_callback(app, dash_duo, component_id, prop_name):
    @app.callback(
...
def wait_for_element_attribute(
    dash_duo, selector, attribute, value, comp_func, timeout
):
    def func(_):
        try:
            return comp_func(
    ...
    WebDriverWait(dash_duo.driver, timeout).until(func)
