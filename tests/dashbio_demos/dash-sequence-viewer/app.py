...
def header_colors():
    return {"bg_color": "#C50063", "font_color": "white"}
...
def layout():
    ...
    return html.Div(
        ...
        children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            dash_bio.SequenceViewer(
                                id="sequence-viewer", sequenceMaxHeight="100px"
                            ),
                        ],
                    ),
                    html.Div(
                        ...
                        children=html.Div(
                            ...
                            children=[
                                ...
                                html.Br(),
                                html.Div(
                                    id="seq-view-info-aa-comp",
                                    children=[
                                        html.Span(
                                            "Amino acid composition",
                                            className="seq-view-info-element-title",
                                        ),
                                        html.Div(id="test-selection"),
                                    ],
                                ),
                                html.Br(),
                                html.Div(
                                    id="seq-view-info-coverage-clicked",
                                    children=[
                                        html.Span(
                                            "Coverage entry clicked",
                                            className="seq-view-info-element-title",
                                        ),
                                        html.Div(id="test-coverage-clicked"),
                                    ],
                                ),
                                html.Br(),
                                html.Div(
                                    id="seq-view-info-mouse-selection",
                                    children=[
                                        html.Span(
                                            "Mouse selection",
                                            className="seq-view-info-element-title",
                                        ),
                                        html.Div(id="test-mouse-selection"),
                                    ],
                                ),
                                html.Br(),
                                html.Div(
                                    id="seq-view-info-subpart-sel",
                                    children=[
                                        html.Span(
                                            "Subpart selected",
                                            className="seq-view-info-element-title",
                                        ),
                                        html.Div(id="test-subpart-selection"),
                                    ],
                                ),
                            ],
                        ),
                            ...
                            dcc.Tab(
                                ...
                                children=html.Div(
                                    ...
                                    children=[
                                        html.H4(
                                            className="what-is",
                                            children="What is Sequence Viewer?",
                                        ),
                                        html.P(
                                        ...
                                        html.P(
                                            ...
                                            "search that is shown in the component. Upon clicking "
                                            "on a coverage entry, you will be able to see the "
            ...
            dcc.Store(id="coverage-storage", data=initialCov),
            dcc.Store(id="clear-coverage", data=0),
            dcc.Store(id="current-sequence", data=0),
        ],
    )
...
def callbacks(_app):
    ...
    def overlaps_coverage(current_cov, cov_item):
        for i in range(len(current_cov)):
            current_range = list(range(current_cov[i]["start"], current_cov[i]["end"]))
            item_range = list(range(cov_item["start"], cov_item["end"]))
    ...
    @_app.callback(
        ...
        [Input("coverage-storage", "data"), Input("selection-or-coverage", "value")],
    )
    ...
    @_app.callback(
        Output("sel-slider", "value"), [Input("sequence-viewer", "sequence")]
    )
    def reset_selection(_):
        return [0, 0]

    @_app.callback(
        Output("sel-region-low", "value"), [Input("sequence-viewer", "sequence")]
    )
    def reset_selection_low(*_):
        return 0

    @_app.callback(
        Output("sel-region-high", "value"), [Input("sequence-viewer", "sequence")]
    )
    ...
    @_app.callback(
        Output("sel-region-slider", "style"), [Input("sel-slider-or-input", "value")]
    )
    def show_hide_slider(slider_input):
        return {"display": "block"} if slider_input == "slider" else {"display": "none"}

    @_app.callback(
        Output("sel-region-inputs", "style"), [Input("sel-slider-or-input", "value")]
    )
    def show_hide_inputs(slider_input):
        return {"display": "block"} if slider_input == "input" else {"display": "none"}

    @_app.callback(
        Output("cov-options", "style"), [Input("selection-or-coverage", "value")]
    )
    ...
    def update_num_entries(entries):
        return "Number of entries: {}".format(len(entries))

    @_app.callback(
        Output("fasta-entry-dropdown", "options"),
        [Input("upload-fasta-data", "contents"), Input("preloaded-sequences", "value")],
    )
    def update_protein_options(upload_contents, preloaded):

        dropdown_options = [{"label": 1, "value": 0}]
        ...
        if isinstance(proteins, list):
            ...
            for i in range(len(proteins)):
                dropdown_options.append(dict(label=i + 1, value=i))
        return dropdown_options

    @_app.callback(Output("sel-slider", "max"), [Input("sequence-viewer", "sequence")])
    ...
    @_app.callback(
        Output("sel-region-high", "max"), [Input("sequence-viewer", "sequence")]
    )
    ...
    @_app.callback(
        Output("sel-region-low", "max"), [Input("sequence-viewer", "sequence")]
    )
        ...
        if v is None or len(v) < 2:
            ...
            if not break_and_return:
                ...
                if not break_and_return:
                    ...
                    aa_counts = [
                        {"aa": seq3(aa), "count": aa_string.count(aa)}
                        for aa in amino_acids
                    ]

                    # sort by most common AA in sequence
                    aa_counts.sort(key=lambda x: x["count"], reverse=True)

                    summary = [
                        html.Tr([html.Td(aac["aa"]), html.Td(str(aac["count"]))])
                        for aac in aa_counts
                    ]

                    # include explanation for translation if necessary
                    if alphabet in ("dna", "rna") and len(summary) > 0:
                        answer = [
                            "(Protein translated from {}: {})".format(
                                alphabet.upper(), aa_string
                            ),
                            html.Table(summary),
                        ]
                    else:
    ...
    def update_cov_clicked(index, current_cov):
        ...
        return "Index: {} Tooltip: {}".format(index, current_cov[index]["tooltip"])
    ...
    @_app.callback(
        ...
        [Input("preloaded-sequences", "value"), Input("upload-fasta-data", "contents")],
    )
