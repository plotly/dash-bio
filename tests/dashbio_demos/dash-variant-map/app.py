...
def layout():
    return html.Div(id='variantmap-body', className='app-body', children=[
        html.Div(id='variantmap-control-tabs', className='control-tabs', children=[
            dcc.Tabs(id='variantmap-tabs', value='what-is', children=[
                # "What is" tab
                dcc.Tab(
                    label='About',
                    value='what-is',
                    children=html.Div(className='control-tab', children=[
                        html.H4(className='what-is', children='What is Variant Map?'),
                        html.P('VariantMap is a genomic structural variant (SV) visualization '
                        ...
                        html.P('VariantMap requires a dataframe object that is generated by '
                ...
                dcc.Tab(
                    ...
                    children=html.Div(className='control-tab', children=[
                        ...
                        html.Div(
                            ...
                            title='Upload your own VariantBreak generated HDF5 dataset here.',
                            children=[
                                dcc.Upload(
                                    id='upload-data',
                        ...
                        html.Div(
                            'Upload label file:',
                            title='This file is used to rename and sort samples.\nExample:\n#Default '
                                  'name<tab>Label\nS1<tab>SampleA\nS3<tab>SampleC\nS2<tab>SampleB',
                            className='app-controls-name'
                        ),
                        html.Div(
                            id='variantmap-tsv-upload',
                            title='Upload a .tsv file to rename and sort samples.\nExample:\n#Default '
                                  'name<tab>Label\nS1<tab>SampleA\nS3<tab>SampleC\nS2<tab>SampleB',
                            children=[
                                dcc.Upload(
                                    id='upload-tsv',
        ...
        dcc.Loading(className='dashbio-loading', children=html.Div(
            ...
            children=[
                ...
                html.Div(
                    id='error-msg',
                    style={'color': 'crimson', 'text-align': 'center', 'font-size': '18px'}
                ),
...
def callbacks(app):
    ...
    def read_data(contents, filename):
        ...
        child_sample = [
            ...
            html.Br(),
            html.Div(
                'Select samples to display and click "SUBMIT":'
            ),
            dcc.Checklist(
                id="select-samples",
                options=[
        ...
        others = sorted(gene_types.difference(['protein_coding', 'lncRNA', 'miRNA', 'snRNA', 'snoRNA']))
        ...
        child_customize = [
            ...
            dcc.Dropdown(
                id='file_filt',
                options=[
                    {'label': name, 'value': name} for name in metadata['filter']
                ],
                value=None,
                multi=True,
            ...
            dcc.Dropdown(
                id='gene_names',
                options=[
                    {'label': name, 'value': name} for name in total_genes
                ],
                value=None,
                multi=True,
            ...
            html.Div(
                ...
                title='Select only variants labeled with these indexes separated by ";"',
                style={'font-weight': 'bold'}
            ),
            dcc.Input(
                id='input_index',
            ...
            dcc.Checklist(
                id="select-genetype",
                options=[
                    {'label': name, 'value': name} for name in main_types
                ]
            ),
            html.Br(),
            html.Div(
            ...
            dcc.Checklist(
                id="select-feature",
                options=[
                    {'label': name, 'value': name} for name in features
                ]
            ),
            html.Br(),
            html.Div(
            ...
            dcc.Checklist(
                id="select-annotation",
                options=[
                    {'label': name, 'value': name} for name in annotes
                ],
            ),
            html.Br(),
            html.Div(
    ...
    @app.callback(
        [Output('sample_labels', 'data'),
         Output('sample_order', 'data')],
    ...
    @app.callback(
        ...
        [Input('count-store', 'data'),
         Input('custom-store', 'data')]
    )
    def make_slider(row_counts, custom_config):
        ...
        child_slider = [
            html.Div(
                'Sections of %i Variants' % entries,
                style={
                    'textAlign': 'center',
                }
            ),
    ...
    def store_custom(n_clicks, sample_filt, file_filt, index_str, gene_names, gene_types, features, annotes, entries, data,
                     name_dict):
        ...
        #     raise HaltCallback('ERROR: Some variant indexes are not found in the data.')
        try:
    ...
    # Callback upon slider selection, data submit button click and storing of sample label data
    @app.callback(
        [Output('variantmap-fig', 'children'),
    ...
    def update_figure(selected_batch, n_clicks, label_dict, data, custom_config, sample_list, sample_order):
        ...
        if custom_config is None:  # If custom_config settings are not provided
            fig = dash_bio.VariantMap(df, batch_no=selected_batch, sample_order=sample_sortlist, sample_names=names_dict)
            ...
            try:
                if custom_config['index_list']:
                    _ = df.loc[custom_config['index_list'], :]  # Test if SV indexes exist in data
