...
def colors_array_layout(data):
    ...
    The colors are based on a gradient bewteen two specified colors. In this case
    they are "red" and "blue". Simpily change these colors if you'd like a different
    gradient.
    '''
    rows = len(data.index)
...
def txt_to_layout(file_one_name='', file_two_name='', append_one='', append_two='', relPath=False, create_local=True):
    ...
    @param append_one
    Append dataset labels of dataset one. Use this if you are using two datasets with the same labels.

    @param append_two
    Append dataset labels of dataset two. Use this if you are using two datasets with the same labels.
